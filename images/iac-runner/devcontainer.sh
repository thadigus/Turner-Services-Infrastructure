...
# Capture host UID/GID for proper file ownership on bind mounts
...
if bash -c 'docker images | grep iac-runner'; then
  ...
  docker build -t iac-runner:latest -f "$git_curr_dir/images/iac-runner/Dockerfile" "$git_curr_dir"
fi
...
docker run --rm -it \
  ...
  -e HOME=/workspace \
  -e PATH="/home/turneradmin/.pulumi/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" \
  -v "$SSH_AUTH_SOCK:/ssh-agent" \
  -e SSH_AUTH_SOCK=/ssh-agent \
  -v "$git_curr_dir:/workspace" \
  "${pulumienv_args[@]}" \
  iac-runner:latest
